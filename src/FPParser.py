...
#		self.findRootPage()
		...
		while (self.__pageNext) :
			...
			if self.__grilleCounter % frequenceSvg_p == 0 :
				print "grille %d, save xcl file" % self.__grilleCounter
		...
		for key_l in keylist_l :
			...
			if len (grille_l['cote1']) == 0 : # recuperer les cotes
				...
				for indexGame_l in range(0, len_l):
					myAnnabet = AnnabetParser.AnnabetParser()
					team1_l = grille_l['team1'][indexGame_l] 
					team2_l = grille_l['team2'][indexGame_l] 
					myAnnabet.setTeam1(team1_l)
					myAnnabet.setTeam2(team2_l)
					myAnnabet.setDate(grille_l['date'])
					odds_l = myAnnabet.getOdds()
...
class FPPronoParser(HTMLParser): 
	...
	def handle_starttag(self, tag, attrs):
		global currentGrille
		if tag == "div" and len(attrs) == 1 and self.__nextTitle == 0 :
			if attrs[0][0] == "class" and attrs[0][1] == "titre-lc ombrage" :
				self.__nextType = True
		elif tag == "div" and len(attrs) == 1 and self.__nextTitle == 1 :
			if attrs[0][0] == "class" and attrs[0][1] == "date-lc" :
				self.__nextTitle = 2
		if tag == "div" and len(attrs) == 1 and self.__nextJackpot == 1 :
	...
	def handle_data(self, data):
		...
		global grilleEmpty
		global grille
		if self.__nextType and data.find("MINI 5") != -1 :
			self.__teamId = 0
			#print "MINI 5 :" 
			self.__nextType = False
			self.__nextTitle = 1
...
#				jackpot = currentGrille['jackpot'] * 10 + int(data)
	...
	def handle_endtag(self, tag):
		if self.__nextJackpot == 2 and tag == "div" :
			self.__nextJackpot = 0
			self.__nextId = 1
